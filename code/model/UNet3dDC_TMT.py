...
class Conv3D_Block(nn.Module):
    def __init__(self, inp_feat, out_feat, kernel=3, stride=1, padding=1, norm=LayerNorm3D, conv_type='normal', residual=None, first=False):
        ...
        if conv_type == 'normal' or first:
            ...
            self.conv1 = nn.Sequential(
                DWconv3D(inp_feat, out_feat, kernel_size=kernel, stride=stride, padding=padding, bias=True),
                norm(out_feat),
                nn.GELU())
        elif conv_type == 'deformable':
            self.conv1 = nn.Sequential(
                DeformConv3d(inp_feat, out_feat, kernel_size=kernel, stride=stride, padding=padding, bias=True),
                norm(out_feat),
                nn.GELU())

        # For simplicity, using the same type for the second convolution, adjust if needed
