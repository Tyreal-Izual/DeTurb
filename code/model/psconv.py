...
class PSConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, parts=1, bias=True):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, 1, groups=1, bias=bias)
...
class PSConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, parts=1, bias=True):
        super(PSConv3d, self).__init__()
        self.gwconv = nn.Conv3d(in_channels, out_channels, kernel_size, stride, dilation, dilation, groups=parts, bias=bias)
        self.gwconv_shift = nn.Conv3d(in_channels, out_channels, kernel_size, stride, 2 * dilation, 2 * dilation, groups=parts, bias=bias)
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size, stride, padding, 1, groups=1, bias=bias)
        ...
        self.mask = torch.zeros(self.conv.weight.shape, dtype=torch.bool).cuda()
        _in_channels = in_channels // parts
        _out_channels = out_channels // parts
        for i in range(parts):
            self.mask[i * _out_channels: (i + 1) * _out_channels, i * _in_channels: (i + 1) * _in_channels, : , :, :] = 1
