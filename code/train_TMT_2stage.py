...
def get_args():
    ...
    parser.add_argument('--val-period', '-vp', dest='val_period', type=int, default=5000,
...
def main():
    ...
    train_dataset = DataLoaderTurbVideo(args.train_path, num_frames=args.num_frames, patch_size=args.patch_size,
                                        noise=0.0001, is_train=True)
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True,
                              num_workers=args.num_workers, drop_last=True, pin_memory=True)

    val_dataset = DataLoaderTurbVideo(args.val_path, num_frames=args.num_frames, patch_size=args.patch_size,
                                      noise=0.0001, is_train=False)
    val_loader = DataLoader(dataset=val_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                            drop_last=True, pin_memory=True)

    model = TMT_MS(num_blocks=[2, 3, 3, 4], num_refinement_blocks=2, n_frames=args.num_frames, att_type='shuffle').to(
        device)
    ...
    start_iter = 1
    warmup_iter = 10000
    scheduler_cosine = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, total_iters - warmup_iter,
                                                                      eta_min=1e-6)
    scheduler = GradualWarmupScheduler(optimizer, multiplier=1, total_epoch=warmup_iter,
    ...
    for epoch in range(1000000):
        for data in train_loader:
            ...
            if iter_count % args.log_frequency == 0:
                ...
                logging.info(
                    'Training: iters {:d}/{:d} -Time:{:.6f} -LR:{:.7f} -Loss {:8f} -PSNR: {:.2f} dB; SSIM: {:.4f}'.format(
                        ...
                        current_loss / args.log_frequency, psnr, ssim))
                current_start_time = time.time()
                current_loss = 0
            ...
            if iter_count > 0 and iter_count % args.val_period == 0:
                ...
                logging.info(
                    'Validation: Iters {:d}/{:d} - Loss {:8f} - PSNR: {:.2f} dB; SSIM: {:.4f}'.format(iter_count,
                                                                                                      total_iters,
                                                                                                      eval_loss / s,
                                                                                                      psnr, ssim))
                if psnr > best_psnr:
