        ...
        for data in train_loader:
            ...
            if iter_count>0 and iter_count % args.val_period == 0:
                ...
                for s, data in enumerate(val_loader):
                    ...
                    with torch.no_grad():
                        try:
                            output_ = model(input_.permute(0, 2, 1, 3, 4)).permute(0, 2, 1, 3, 4)
                        except RuntimeError as e:
                            output_ = model(input_).permute(0, 2, 1, 3, 4)

                    with torch.no_grad():
                        try:
                            output = model(output_.permute(0, 2, 1, 3, 4)).permute(0, 2, 1, 3, 4)
                        except RuntimeError as e:
                            output = model(output_).permute(0, 2, 1, 3, 4)
