...
from model.TMT_Swin3D import TMT_MS
...
def get_args():
    ...
    parser.add_argument('--patch-size', '-ps', dest='patch_size', type=int, default=256, help='Patch size(minimum 128)')
    ...
    parser.add_argument('--num_frames', type=int, default=6, help='number of frames for the model')
    parser.add_argument('--num_workers', type=int, default=4, help='number of workers in dataloader')
    parser.add_argument('--train_path', type=str, default='turb_syn_videos/train', help='path of training imgs')
    parser.add_argument('--val_path', type=str, default='turb_syn_videos/test', help='path of validation imgs')
    parser.add_argument('--march', type=str, default='normal', help='model architecture')
    parser.add_argument('--load', '-f', type=str, default=False, help='Load model from a .pth file')
    parser.add_argument('--log_path', type=str, default='data/train_log', help='path to save logging files and images')
...
def main():
    ...
    if gpu_count > 1:
        model = torch.nn.DataParallel(model, device_ids=[i for i in range(gpu_count)]).to(device)
    ...
    for epoch in range(1000000):
        ...
        for data in train_loader:
            ...
            s4 = time.time()
            print(f'img: {s4 - s3}, backward: {s3 - s2}, forward:{s2 - s1}, data: {s1 - s0}')
            s0 = time.time()
            ...
            if iter_count % args.log_frequency == 0:
                ...
                logging.info('Training: iters {:d}/{:d} -Time:{:.6f} -LR:{:.7f} -Loss {:8f} -PSNR: {:.2f} dB; SSIM: {:.4f}'.format(iter_count, total_iters, time.time()-current_start_time, optimizer.param_groups[0]['lr'], current_loss/args.log_frequency, psnr, ssim))
                current_start_time = time.time()
                current_loss = 0
            ...
            if iter_count>start_iter and iter_count % args.print_period == 0:
                ...
                # logging.info(
                #     'Training: iters {:d}/{:d} -Time:{:.6f} -LR:{:.7f} -Loss {:8f} -PSNR: {:.2f} dB; SSIM: {:.4f}'.format(
                #         iter_count, total_iters, time.time() - current_start_time, optimizer.param_groups[0]['lr'],
                #         current_loss / args.print_period, psnr, ssim))
                # print(
                #     'Training: iters {:d}/{:d} -Time:{:.6f} -LR:{:.7f} -Loss {:8f} -PSNR: {:.2f} dB; SSIM: {:.4f}'.format(
                #         iter_count, total_iters, time.time() - current_start_time, optimizer.param_groups[0]['lr'],
                #         current_loss / args.print_period, psnr, ssim))
                ...
                for s, data in enumerate(val_loader):
                    ...
                    for b in range(output.shape[0]):
                        for i in range(output.shape[1]):
                            ...
                            if s % 250 == 0:
                                pg_save = Image.fromarray(np.uint8(np.concatenate((inp, img, img_gt), axis=1))).convert(
                                    'RGB')
