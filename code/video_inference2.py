...
if __name__ == '__main__':
    ...
    for i in range(1, num_chunk):
        if i == num_chunk - 1:
            test_frame_info.append({'start': total_frames - args.temp_patch,
                                    'range': [i * (args.temp_patch // 2) + (args.temp_patch // 4), total_frames]})
        else:
            test_frame_info.append({'start': i * (args.temp_patch // 2),
                                    'range': [i * (args.temp_patch // 2) + (args.temp_patch // 4),
                                              i * (args.temp_patch // 2) + (args.temp_patch // 4 * 3)]})
    ...
    for i in range(num_chunk):
        ...
        inp_imgs = [Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)) for img in
                    all_frames[in_range[0]:in_range[1]]]
        ...
        if max(hh, ww) < patch_size:
            first_pass_output = net(input_)
        else:
            first_pass_output = test_spatial_overlap(input_, net, patch_size)

        if max(hh, ww) < patch_size:
            second_pass_output = net(first_pass_output)
        else:
            second_pass_output = test_spatial_overlap(first_pass_output, net, patch_size)

        second_pass_output = second_pass_output.permute(0, 2, 1, 3, 4)
        input_ = input_.permute(0, 2, 1, 3, 4)

        for j in range(out_range[0] - in_range[0], out_range[1] - in_range[0]):
            out = cv2.cvtColor(restore_PIL(second_pass_output, 0, j), cv2.COLOR_RGB2BGR)
    ...
    if args.save_video:
        if args.concatenate_input:
            output_writer = cv2.VideoWriter(args.out_path, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps,
                                            (ww * 2, hh))
