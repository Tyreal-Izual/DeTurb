...
def get_args():
    parser = argparse.ArgumentParser(description='Batch video inference with overlapping patches')
...
if __name__ == '__main__':
    ...
    for video_file in video_files:
        ...
        if args.save_images:
            frame_dir = os.path.join(args.out_folder, os.path.splitext(video_file)[0])
        ...
        if args.save_video:
            if args.concatenate_input:
                output_writer = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps, (ww * 2, hh))
                for fid, frame in enumerate(out_frames):
                    output_writer.write(np.concatenate((inp_frames[fid], frame), axis=1))
            else:
                output_writer = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps, (ww, hh))
                for fid, frame in enumerate(out_frames):
